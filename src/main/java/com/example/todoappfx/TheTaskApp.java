...
public class TheTaskApp extends Application {
    @Override
...
//    }
    ...
    private void displayTasks(Stage primaryStage) {
        ...
        listView.setCellFactory(param -> new TaskListCell(primaryStage));

        // Create UI components
        Button addButton = new Button("+");
        addButton.getStyleClass().add("button_blue");  // Add your CSS class for styling
        addButton.setStyle("-fx-font-size: 30px");
        addButton.setPadding(new Insets(0, 10, 0, 10));
        // Add action for the "+" button
        addButton.setOnAction(event -> displayEnterTaskScreen(primaryStage));

        // Create the scene and apply styles
        VBox root = new VBox(5, addButton, listView);  // Adjust the spacing (10 in this case)
        root.setStyle("-fx-background-color: #e89292;");  // Set your desired background color
        ...
        scrollPane.setStyle("-fx-background-color: #d01a1a;");
    ...
    private static class TaskListCell extends ListCell<Tasks> {
        ...
        public TaskListCell(Stage primaryStage) {
        ...
        protected void updateItem(Tasks task, boolean empty) {
            ...
            if (empty || task == null) {
                ...
                HBox hbox = new HBox(30, detailsLabel, checkBox, deleteButton);  // Adjust the spacing (20 in this case)
                hbox.setPadding(new Insets(5));
                hbox.setStyle(
                        "-fx-background-color: #decc57;" +  // Set your desired background color
                                "-fx-border-radius: 50;" +  // Optional: Add rounded corners
                                "-fx-padding: 20;"  // Optional: Add padding
                );
                setGraphic(hbox);
            }
        }
    ...
    private static void completedOrNot(int taskID, CheckBox checkBox) {
        ...
        try (Connection connection = DriverManager.getConnection(jdbcUrl, username, password)) {
            String updateQuery = "UPDATE task_details SET Completed = ? WHERE taskID = ?";
            try (PreparedStatement preparedStatement = connection.prepareStatement(updateQuery)) {
                preparedStatement.setInt(1, completedValue);
                preparedStatement.setInt(2, taskID);
                int rowsAffected = preparedStatement.executeUpdate();
                System.out.println("Rows affected: " + rowsAffected);
                System.out.println("Task with TaskID " + taskID + " marked as " + (checkBox.isSelected() ? "Completed" : "Incomplete"));
            } catch (SQLException e) {
                e.printStackTrace();
            }
        } catch (SQLException e) {
    ...
    private static void deleteTask(int taskID) {
        ...
        try (Connection connection = DriverManager.getConnection(jdbcUrl, username, password)) {
            // Create a PreparedStatement
            String deleteQuery = "DELETE FROM task_details WHERE taskID = ?";
